 ...
 * Limit: Due to extensiveness of algorithm, let's keep it limited to 10 words to begin with
 */
void swapper(string, uint);
...
uint main()
{
	string inp;
	start:
	cout << "Please enter a string that has between 1-11 letters.\n";
	cin >> inp;
	uint len = inp.length();
	if(len > 11)
	{
		cout << "Enter less than 11 letters" <<endl;
		goto start;
	}
	std::ofstream ofs;
...
void swapper(string inp, uint pos)
{
	uint len=inp.length();
...
bool checkStringInString(string s1, string s2) //Check for string s2 in string s1
{
	bool result;
	uint n = 0;
	uint l = (s1.length() == s2.length()) ? 1 : (s1.length() - s2.length());
	for(uint i=0; i < l; i++)
	{
